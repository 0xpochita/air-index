...
const matches = (entry: LiveIndex, query: string): boolean => {
  ...
  return (
    ...
    entry.ensName.toLowerCase().includes(needle) ||
...
export const IndexSelect = ({
  ...
  const priceUsd = (slug: string) =>
    toFloat(sharePrices[slug] ?? 0n, quote.decimals) *
              ...
              <div className="mt-2 min-h-0 flex-1 overflow-y-auto">
                {visible.length === 0 ? (
                  ...
                  visible.map((entry, index) => {
                    const isSelected = entry.slug === value.slug;
                      ...
                      >
                        ...
                        <span className="shrink-0 text-right">
                          <span className="block text-sm font-semibold tabular-nums text-ink">
                            {formatUsd(priceUsd(entry.slug))}
                          </span>
                          <span className="block text-xs tabular-nums text-ink-subtle">
                            {formatAmount(
                              toFloat(shares[entry.slug] ?? 0n, SHARE_DECIMALS),
                            )}
                          </span>
                        </span>
                      </motion.button>
